...
@Service
@Transactional
public class SolicitudProgramaServiceImpl implements SolicitudProgramaService {
	...
	public SolicitudProgramaServiceImpl(SolicitudProgramaRepository solicitudProgramaRepository,
			SolicitudProgramaMapper solicitudProgramaMapper, ProgramaSocialRepository programaSocialRepository,
			CategoriaRepository categoriaRepository, EstadoPublicacionRepository estadoPublicacionRepository,
			OngRepository ongRepository) {

		this.solicitudProgramaRepository = solicitudProgramaRepository;

		this.solicitudProgramaMapper = solicitudProgramaMapper;
	...
	@Override
	public SolicitudPrograma crearSolicitud(Long ongId, SolicitudProgramaRequest request) {

		SolicitudPrograma solicitud = solicitudProgramaMapper.toEntity(request);
		...
		Ong ong = ongRepository.findById(ongId).orElseThrow(() -> new RuntimeException("ONG no encontrada"));

		solicitud.setOng(ong);

		EstadoPublicacion estadoPendiente = estadoPublicacionRepository.findByNombre("PENDIENTE")
	...
	public void aprobar(Long id) {
		...
		SolicitudPrograma solicitud = solicitudProgramaRepository.findById(id)
				.orElseThrow(() -> new RuntimeException("Solicitud de programa no encontrada"));

		EstadoPublicacion estadoPublicado = estadoPublicacionRepository.findByNombre("PUBLICADO")
				.orElseThrow(() -> new RuntimeException("Estado PUBLICADO no encontrado"));

		ProgramaSocial programa = new ProgramaSocial();

		programa.setOng(solicitud.getOng());

		programa.setCategoria(solicitud.getCategoria());

		programa.setNombre(solicitud.getTitulo());

		programa.setDescripcion(solicitud.getDescripcion());

		programa.setObjetivoMonto(solicitud.getObjetivoMonto());

		programa.setMontoActual(BigDecimal.ZERO);

		programa.setEstado(estadoPublicado);

		programa = programaSocialRepository.save(programa);

		solicitud.setReferenciaId(programa.getId());

		solicitud.setEstado(estadoPublicado);
	...
	public void rechazar(Long id) {
		...
		EstadoPublicacion estadoRechazado = estadoPublicacionRepository.findByNombre("RECHAZADO")
