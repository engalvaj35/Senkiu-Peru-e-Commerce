...
public class SolicitudOrganizacionServiceImpl implements SolicitudOrganizacionService {
    ...
    public SolicitudOrganizacionServiceImpl(
                SolicitudOrganizacionRepository repo,
    ...
    // =========================
    @Override
    public void crearSolicitudEmpresa(
            Long usuarioId,
        ...
        if (
                request.getNumeroCuenta() != null &&
                !request.getNumeroCuenta().isBlank() &&
                cuentaRepository.existsByNumeroCuenta(
        ...
        if (
                ...
                cuentaRepository.existsByCci(
                        request.getCci().trim()
                )
        ) {

                throw new RuntimeException(
                        "El CCI ya está registrado"
                );
        }
        ...
        EstadoOrganizacion estado = estadoRepo.findByNombre("PENDIENTE")
                .orElseThrow();
    ...
    @Override
        ...
        guardarCertificado(
                sol,
                request.getNombreCertificado(),
                archivo
        );
        }

    // =========================
    // APROBAR / RECHAZAR SOLO SOLICITUD
    // =========================
    @Override
        public void aprobarSolicitud(Long id) {

        SolicitudOrganizacion sol = repo.findById(id)
                .orElseThrow();
        ...
        if (sol.getTipo().equals("EMPRESA")) {
                Empresa empresa =
                        empresaService.crearDesdeSolicitud(sol);

                cuentaBancariaService
                        .crearDesdeSolicitudEmpresa(
                                empresa.getId(),
                                sol
                        );
        }
        ...
        if (sol.getTipo().equals("ONG")) {
                Ong ong =
                        ongService.crearDesdeSolicitud(sol);

                cuentaBancariaService
                        .crearDesdeSolicitudOng(
                                ong.getId(),
                                sol
                        );
        }
    ...
    public void rechazarSolicitud(Long id) {
        ...
        EstadoOrganizacion rechazo = estadoRepo.findByNombre("RECHAZADO")
                .orElseThrow();
