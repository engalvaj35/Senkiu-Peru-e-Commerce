...
public class TicketServiceImpl implements TicketService {
	...
	public TicketServiceImpl(TicketRepository ticketRepository, UsuarioRepository usuarioRepository,
			RifaRepository rifaRepository, EstadoTicketRepository estadoTicketRepository, PagoRepository pagoRepository,
			EstadoPagoRepository estadoPagoRepository, WalletService walletService, PagoService pagoService) {
	...
	public List<TicketResponse> obtenerMisTickets(Long usuarioId) {
		...
		return ticketRepository.findByUsuario(usuario).stream().map(this::mapToResponse).toList();
	}

	@Override
	@Transactional
	public Long comprarTickets(Long usuarioId, Long rifaId, int cantidad, MetodoPago metodoPago) {

		Usuario usuario = usuarioRepository.findById(usuarioId)
				.orElseThrow(() -> new RuntimeException("Usuario no encontrado"));

		Rifa rifa = rifaRepository.findById(rifaId).orElseThrow(() -> new RuntimeException("Rifa no encontrada"));

		if (!"ACTIVA".equalsIgnoreCase(rifa.getEstado().getNombre())) {

			throw new RuntimeException("La rifa no está activa");
		}

		if (rifa.getFechaFin().isBefore(java.time.LocalDateTime.now())) {

			throw new RuntimeException("La rifa ya finalizó");
		}

		int disponibles = rifa.getStockTickets() - rifa.getTicketsVendidos();
		...
		EstadoTicket estadoPendiente = estadoTicketRepository.findByNombre("PENDIENTE_PAGO").orElseThrow();

		EstadoPago estadoPago = estadoPagoRepository.findByNombre("PENDIENTE").orElseThrow();
		...
		pago.setMonto(rifa.getPrecioTicket().multiply(BigDecimal.valueOf(cantidad)));
		...
		if (metodoPago == MetodoPago.WALLET) {

			walletService.restarCreditos(usuarioId, pago.getMonto());
	...
	private TicketResponse mapToResponse(Ticket ticket) {
		...
		return new TicketResponse(ticket.getId(), ticket.getCodigo(), ticket.getRifa().getTitulo(),
				ticket.getEstado().getNombre(), ticket.getPago() != null ? ticket.getPago().getId() : null);
	}
