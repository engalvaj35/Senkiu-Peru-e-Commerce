...
public class EmpresaServiceImpl implements EmpresaService {
	...
	public EmpresaServiceImpl(EmpresaRepository repository, UsuarioRepository usuarioRepository,
	...
	public Empresa obtenerPorUsuario(Long usuarioId) {
		return repository.findByUsuarioId(usuarioId).orElseThrow(() -> new RuntimeException("Empresa no existe"));
	}
	...
	public Empresa crearDesdeSolicitud(SolicitudOrganizacion sol) {
		...
		Usuario user = usuarioRepository.findById(sol.getUsuario().getId())
	...
	public void suspender(Long empresaId) {
		Empresa empresa = repository.findById(empresaId).orElseThrow(() -> new RuntimeException("Empresa no existe"));
		EstadoOrganizacion estado = estadoRepo.findByNombre("SUSPENDIDO").orElseThrow();
	...
	public void activar(Long empresaId) {
		Empresa empresa = repository.findById(empresaId).orElseThrow(() -> new RuntimeException("Empresa no existe"));
		EstadoOrganizacion estado = estadoRepo.findByNombre("APROBADO").orElseThrow();
		empresa.setEstado(estado);
		repository.save(empresa);
	}
