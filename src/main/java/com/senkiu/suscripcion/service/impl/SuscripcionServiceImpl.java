...
public class SuscripcionServiceImpl implements SuscripcionService {
	...
	public SuscripcionServiceImpl(SuscripcionRepository repository, UsuarioRepository usuarioRepository,
			EstadoSuscripcionRepository estadoRepository, PagoService pagoService) {
	...
	public Suscripcion crearSuscripcion(Long usuarioId, String plan) {
		...
		repository.findByUsuario(usuario).ifPresent(s -> {

			if ("ACTIVA".equals(s.getEstado().getNombre()) && s.getFechaFin().isBefore(LocalDateTime.now())) {

				EstadoSuscripcion expirada = estadoRepository.findByNombre("EXPIRADA").orElseThrow();
		...
		repository.findByUsuarioAndEstado_NombreIn(usuario, java.util.List.of("ACTIVA", "PENDIENTE")).ifPresent(s -> {
