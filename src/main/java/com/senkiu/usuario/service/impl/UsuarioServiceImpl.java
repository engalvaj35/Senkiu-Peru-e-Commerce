...
public class UsuarioServiceImpl implements UsuarioService {
	...
	@Override
	public UsuarioPerfilResponse completarPerfil(Long usuarioId, UsuarioPerfilRequest request) {

		Usuario user = repository.findById(usuarioId).orElseThrow(() -> new RuntimeException("Usuario no encontrado"));

		if (request.getEmail() != null && !request.getEmail().isBlank()) {

			Optional<Usuario> usuarioEmail = repository.findByEmail(request.getEmail());

			if (usuarioEmail.isPresent() && !usuarioEmail.get().getId().equals(user.getId())) {
		...
		Usuario usuarioGuardado = repository.save(user);
	...
	private UsuarioPerfilResponse convertirDTO(Usuario user) {
