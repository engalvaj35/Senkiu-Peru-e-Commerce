...
public class EntregaController {
    ...
    public EntregaResponse obtenerEntrega(
            @PathVariable Long ordenId
    ) {
        return entregaService.obtenerPorOrden(
                ordenId
        );
    }

    @PutMapping("/orden/{ordenId}")
    public EntregaResponse actualizarEntrega(
            @PathVariable Long ordenId,
            @RequestBody ActualizarEntregaRequest request
    ) {

        return entregaService.actualizarEstado(
                ordenId,
                request
        );
    }
