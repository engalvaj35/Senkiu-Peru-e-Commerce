...
public class ProgramaSocialMapper {
	public ProgramaSocialDTO toDTO(ProgramaSocial programa) {
		...
		if (programa.getObjetivoMonto() != null && programa.getObjetivoMonto().compareTo(BigDecimal.ZERO) > 0
				&& programa.getMontoActual() != null) {
			porcentajeAvance = programa.getMontoActual().multiply(BigDecimal.valueOf(100))
					.divide(programa.getObjetivoMonto(), 2, RoundingMode.HALF_UP);
		}
